...
class AccountMove(models.Model):
    ...
    def auto_applies_before_posting(self):
        for rec in self:
            if (
                ...
                and rec.company_id.l10n_pe_edi_auto_apply_detraction
    ...
    def action_post(self):
        ...
        for rec in self:
            if (
                rec.l10n_pe_edi_is_einvoice
                and rec.l10n_pe_edi_shop_id
                and not rec.l10n_pe_edi_request_id
            ):
                request_id = self.env["l10n_pe_edi.request"].create(
                    {
                        "company_id": rec.company_id.id,
                        "l10n_pe_edi_shop_id": rec.l10n_pe_edi_shop_id.id,
                        "l10n_pe_edi_document_type": rec.l10n_latam_document_type_id.code,
                        "document_number": rec.name,
                        "document_date": rec.invoice_date,
                        "model": rec._name,
                        "res_id": rec.id,
                    }
                )
                rec.l10n_pe_edi_request_id = request_id.id
        return res
